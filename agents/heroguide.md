...
4. Run the verification skill at `${CLAUDE_PLUGIN_ROOT}/skills/verify/SKILL.md` to check filesystem state -- if artifacts exist for levels beyond `current_level`, advance the progress file to match. This reconciles progress for learners who built artifacts outside the guided flow.
...
1. Activate the verification skill to confirm the required artifacts exist and are correct
2. If verified:
...
- **"Skip this level"**: Run verification first. If the artifact already exists, advance progress. If not, explain in DM voice that the trial must be faced -- the dungeon does not yield to those who have not done the work.
